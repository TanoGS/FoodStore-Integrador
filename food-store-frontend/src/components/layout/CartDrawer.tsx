        ...
        <div className="flex-1 overflow-y-auto p-6">
          {items.length === 0 ? (
            ...
            <div className="space-y-6">
              {items.map((item) => (
                <div key={`${item.producto.id}-${JSON.stringify(item.producto.personalizacion ?? [])}`} className="flex gap-4 items-center bg-white border border-slate-100 p-3 rounded-2xl shadow-sm">
                  {/* Foto del producto */}
                  ...
                  <div className="flex-1">
                    ...
                    {/* Personalización: ingredientes quitados */}
                    {item.producto.personalizacion && item.producto.personalizacion.length > 0 && (
                      <p className="text-xs text-red-500 font-medium mb-1">
                        <span className="font-bold">Sin:</span>{' '}
                        {item.producto.personalizacion.map(p => p.nombre).join(', ')}
                      </p>
                    )}
